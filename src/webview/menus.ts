...
export function updateStatusBar(
  ...
  if (contextUsed !== undefined) {
    ...
    if (size > 0) {
      ...
      // Total in the headline, cache share alongside (option A of the four
      // that were sketched here). "How full is my window?" is the question
      // people actually ask, and a fresh-token headline reads as a broken
      // counter on a warm cache, where fresh = 0 is legitimate.
      //
      // See renderTokenDisplay in src/tokenDisplay.ts — it lives outside this
      // module so it can be unit-tested; webview/ is excluded from the main
      // tsconfig because it touches the DOM.
